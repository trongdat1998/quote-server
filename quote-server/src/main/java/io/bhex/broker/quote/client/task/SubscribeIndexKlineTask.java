...
@Slf4j
public class SubscribeIndexKlineTask extends AbstractSubscribeTask {
        ...
        try {
            ...
            for (KlineIntervalEnum value : KlineIntervalEnum.values()) {
                List<KLine> kLineList =
                    dataServiceRepository.getIndexKlineList(symbol, value.interval, KLINE_CACHE_SIZE, defaultOrgId);
            ...
            for (Map.Entry<KlineIntervalEnum, List<KLine>> klineIntervalEnumListEntry : cacheListMap.entrySet()) {
                ...
                IndexKlineListener indexKlineListener = context.getBean(BeanUtils.getIndexKlineBeanName(symbol,
                    intervalEnum.interval), IndexKlineListener.class);
                indexKlineListener.clearSnapshot();
                if (CollectionUtils.isNotEmpty(kLineList)) {
                    indexKlineListener.reloadHistoryData(kLineList);
                }
            }
