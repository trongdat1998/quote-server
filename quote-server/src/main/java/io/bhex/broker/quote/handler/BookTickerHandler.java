...
@Component
public class BookTickerHandler implements ITopicHandler {
    private final SymbolRepository symbolRepository;
    private final ApplicationContext context;

    @Autowired
    public BookTickerHandler(SymbolRepository symbolRepository, ApplicationContext context) {
    ...
    public void handle(ChannelHandlerContext ctx, ClientMessageV2 msg) {
        ...
        try {
            ...
            .getBean(BeanUtils.getBookTickerV2BeanName(sharedExchangeId, symbolId), BookTickerListener.class);
            EventEnum event = EventEnum.valueOF(msg.getEvent());
            if (EventEnum.SUB.equals(event)) {
                bookTickerListener.subscribe(ctx.channel(), msg);
            } else if (EventEnum.CANCEL.equals(event)) {
                bookTickerListener.unsubscribe(ctx.channel(), msg);
            }
    ...
    public boolean isSameTopic(ClientMessageV2 msg) {
        return StringUtils.equalsIgnoreCase(TopicEnum.bookTicker.name(), msg.getTopic());
    }
}
