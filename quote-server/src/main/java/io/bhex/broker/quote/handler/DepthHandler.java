...
@Component
public class DepthHandler implements ITopicHandler {
    private final SymbolRepository symbolRepository;
    private final ApplicationContext context;

    @Autowired
    public DepthHandler(SymbolRepository symbolRepository, ApplicationContext context) {
    ...
    public void handle(ChannelHandlerContext ctx, ClientMessageV2 msg) {
        ...
        try {
            ...
            DepthListener depthListener = context.getBean(BeanUtils.getDepthV2BeanName(sharedExchangeId, symbolId), DepthListener.class);
            EventEnum event = EventEnum.valueOF(msg.getEvent());
            if (EventEnum.SUB.equals(event)) {
                depthListener.subscribe(ctx.channel(), msg);
            } else if (EventEnum.CANCEL.equals(event)) {
                depthListener.unsubscribe(ctx.channel(), msg);
            }
    ...
    public boolean isSameTopic(ClientMessageV2 msg) {
        return StringUtils.equalsIgnoreCase(TopicEnum.depth.name(), msg.getTopic());
    }
}
